...
void pickUpChopsticks(int threadIndex) {
    ...
    while(1) {
        ...
        if(!pthread_mutex_trylock(&chopsticks[threadIndex])){
            //if get left one, try to get right chopsticks
            if(!pthread_mutex_trylock(&chopsticks[(threadIndex+1)%max])){
            return ; //got both chopsticks
            } else {
            pthread_mutex_unlock(&chopsticks[threadIndex]);
            //if did not get right one, release left first; then re-try
            }
        }
    }
    return;
